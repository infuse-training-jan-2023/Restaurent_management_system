...
async def mock_fetch_all_tables(mocker):
    ...
    mocker.patch('repository.table_repository.fetch_all_tables', return_value=cursor)
...
async def test_invalid_update_table(mocker):
    mock = mocker.patch('repository.table_repository.fetch_all_tables', return_value = table_data)
...
async def mock_create_table(mocker):
    cursor = mocker.MagicMock()
    cursor.__aiter__.return_value = iter(table_data)
    mocker.patch('repository.table_repository.create_table', return_value=cursor)
