...
async def test_insert_order_history(mocker):
    mocker.patch(
        'repository.checkout_repository.insert_order_history', return_value=data)
...
async def test_delete_cart(mocker):
    mocker.patch('repository.checkout_repository.delete_cart',
                 return_value={"msg": "user deleted successfully"})
    username = "user2"
