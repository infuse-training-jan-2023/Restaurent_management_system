...
axios.delete(`http://localhost:8000/cart/${user}/${deleteItem.item_name}`)
.then(res => {
...
<nav className="flex fixed w-full top-0 items-center justify-between border-4 border-white rounded-lg flex-wrap bg-teal-500 p-6">
...
{  !user && (<button className="bg-primary px-6 py-3 text-white poppins rounded-full ring-blue-300 focus:outline-none focus:ring-4 transform transition duration-700 hover:scale-105 py-3 mr-2 " onClick={show_login_modal}>
SignIn
</button>)}
...
<div>
{ user && (<span>
<div className="text-3xl font-medium text-white-900 dark:text-white  px-4 block poppins rounded-full mt-4 lg:inline-block lg:mt-0 text-white">
Hi {user} <span><Avatar size='large' style={{ backgroundColor: 'blue' }} icon={<UserOutlined />} /></span>
</div>
